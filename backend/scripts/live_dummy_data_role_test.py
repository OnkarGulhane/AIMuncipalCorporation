...
def main():
    print("=" * 70)
    print("🚀 LIVE MULTI-ROLE & DUMMY DATA LIFECYCLE VERIFICATION")
    print("=" * 70)
    ...
    print(f"✅ Loaded {len(categories)} municipal categories: {list(cat_map.keys())}")
    ...
    print("\n📝 Submitting 3 new civic grievance reports as Citizen...")
    for report in dummy_reports:
        ...
        if code in [200, 201]:
            ...
            print(f"  ✅ Case Filed: #{case.get('case_number', case['id'])} - '{case['title']}' (Status: {case['status']})")
        else:
            print(f"  ❌ Failed to create case: {code} {case}")
    ...
    print(f"✅ Citizen Case Directory contains {len(citizen_cases)} cases.")
    ...
    print(f"\n🧠 Running AI Triage on Case #{target_case_id}...")
    code, ai_analysis = api_call(f"/cases/{target_case_id}/ai-analysis", method="POST", token=operator_token)
    if code == 200:
        print(f"  ✅ AI Analysis Result: Category={ai_analysis.get('predicted_category_code')}, Severity={ai_analysis.get('suggested_severity')}, Priority={ai_analysis.get('suggested_priority')}")
        print(f"  Confidence Score: {ai_analysis.get('confidence_score')}")

    # Operator generates AI communication draft
    print(f"\n💬 Operator generating AI Citizen Communication Draft...")
    draft_req = {
        "audience": "citizen",
    ...
    if code == 200:
        print(f"  ✅ AI Draft Generated (Subject: '{draft_res.get('subject')}')")
        print(f"  Message: \"{draft_res.get('body_text', '')[:100]}...\"")

    # Operator transitions: reported -> understood -> assigned
    print(f"\n🔄 Operator transitioning Case #{target_case_id}: reported -> understood...")
    ...
    print(f"  ✅ Case State: {c1.get('status')}")

    print(f"🔄 Operator assigning Squad: understood -> assigned...")
    ...
    print(f"  ✅ Case Assigned to Squad: Status={c2.get('status')}, TeamID={c2.get('team_id')}")
    ...
    if code == 200:
        print(f"  ✅ SLA Clock Active: Target={sla_data.get('time_remaining_formatted', 'On Track')}")

    # Team Lead advances: assigned -> investigated -> action_taken -> resolution_proposed
    print(f"🛠️ Team Lead inspecting site: assigned -> investigated...")
    ...
    print(f"  ✅ Case State: {c3.get('status')}")

    print(f"🛠️ Team Lead performing repairs: investigated -> action_taken...")
    ...
    print(f"  ✅ Case State: {c4.get('status')}")

    print(f"🛠️ Team Lead submitting completion: action_taken -> resolution_proposed...")
    ...
    print(f"  ✅ Case State: {c5.get('status')}")
    ...
    if code == 200:
        print(f"  ✅ Citizen Confirmed! Case closed cleanly with state: {conf_res.get('status')}")
    e    # -------------------------------------------------------------
    ...
    if code == 200:
        print(f"  ✅ Manager Analytics: Total Cases={analytics.get('total_cases')}, Open={analytics.get('open_cases')}, Resolved={analytics.get('resolved_cases')}, SLA Compliance={analytics.get('sla_compliance_rate')}%")

    # Manager checks active escalations
    code, escalations = api_call("/escalations", token=manager_token)
    if code == 200:
        print(f"  ✅ Manager Escalations Ledger: {len(escalations)} active monitored cases.")
    ...
    if code == 200:
        ...
        print(f"  ✅ Cryptographic Audit Ledger active: {len(logs)} recent tamper-evident events logged.")

    # Admin checks system stats
    code, system_stats = api_call("/admin/system-stats", token=admin_token)
    if code == 200:
        print(f"  ✅ System Stats: Users={system_stats.get('total_users')}, Cases={system_stats.get('total_cases')}, Active Squads={system_stats.get('total_teams')}")

    print("\n" + "=" * 70)
    print("🎉 ALL 5 ROLES, WORKFLOWS, AI MODULES & DUMMY DATA VERIFIED 100%!")
    print("=" * 70)� ALL 5 ROLES, WORKFLOWS, AI MODULES & DUMMY DATA VERIFIED 100%!")
    print("=" * 70)
